...
for (var i = 0; i < learnstatus_Array.length; i++) {
  for (var j = 0; j < learnstatus_Array[i].data.length; j++) {
    ...
    if (
      remainingTime > 0 &&
      gapTime(now, learnstatus_Array[i].data[j].unlock_at) < 0
    ) {
      if (
        learnstatus_Array[i].data[j].use_attendance &&
        learnstatus_Array[i].data[j].attendance_status != "attendance"
      ) {
      ...
      if (
        learnstatus_Array[i].data[j].type == "assignment" &&
        !learnstatus_Array[i].data[j].completed
      ) {
